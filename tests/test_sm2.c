...
int main(void)
{
    ...
    CHECK((key = sm2_generate_keypair()) != NULL, "keypair");
    CHECK((wrong_key = sm2_generate_keypair()) != NULL, "wrong keypair");
    CHECK(sm2_sign(key, message, sizeof(message) - 1,
                   &signature, &signature_len), "sign");
    CHECK(sm2_verify(key, message, sizeof(message) - 1,
                     signature, signature_len), "valid signature rejected");
    memcpy(changed_message, message, sizeof(message));
    changed_message[0] ^= 1u;
    CHECK(!sm2_verify(key, changed_message, sizeof(message) - 1,
    ...
    signature[signature_len - 1] ^= 1u;
    CHECK(!sm2_verify(wrong_key, message, sizeof(message) - 1,
